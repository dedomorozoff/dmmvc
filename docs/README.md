# DMMVC Documentation

Complete documentation for DMMVC framework.

## Table of Contents

### Getting Started
- [Quick Start](QUICKSTART.md) | [Русский](QUICKSTART.ru.md)
- [Quick Start with CLI](QUICKSTART_CLI.md) | [Русский](QUICKSTART_CLI.ru.md)

### Core Documentation
- [Architecture](ARCHITECTURE.md) | [Русский](ARCHITECTURE.ru.md) - Framework architecture and design patterns
- [CLI Tool](CLI.md) | [Русский](CLI.ru.md) - Code generation tool
- [Examples](EXAMPLES.md) | [Русский](EXAMPLES.ru.md) - Code examples and tutorials
- [File Structure](FILES.md) | [Русский](FILES.ru.md) - Project structure explanation

### Database
- [PostgreSQL Support](POSTGRESQL.md) | [Русский](POSTGRESQL.ru.md) - PostgreSQL setup and configuration
- [Migration Guide](MIGRATION_GUIDE.md) - Database migration between SQLite/MySQL/PostgreSQL

### Deployment
- [Deployment Guide](DEPLOYMENT.md) | [Русский](DEPLOYMENT.ru.md) - Production deployment
- [Docker Guide](DOCKER.md) - Docker and Docker Compose setup

## Quick Links

**For Beginners:**
1. Start with [Quick Start](QUICKSTART.md)
2. Learn [CLI Tool](CLI.md) for code generation
3. Check [Examples](EXAMPLES.md) for common patterns

**For Advanced Users:**
1. Review [Architecture](ARCHITECTURE.md)
2. Setup [PostgreSQL](POSTGRESQL.md) for production
3. Deploy with [Docker](DOCKER.md)

## Language

All documentation is available in:
- English
- Русский (Russian)

## External Resources

- [Go Documentation](https://go.dev/doc/)
- [Gin Framework](https://gin-gonic.com/docs/)
- [GORM](https://gorm.io/docs/)
- [PostgreSQL](https://www.postgresql.org/docs/)

## Contributing

Found an error in documentation? Please open an issue or submit a pull request!
